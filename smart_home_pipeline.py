...
# Fix 1.4: Read ML API port from environment variable so Vite auto-increment
# to the same port no longer silently conflicts with the Python ML server.
ML_PORT = int(os.environ.get("ML_PORT", "5174"))

# DEMO_MODE: lower retrain threshold from 10 → 1 so the learning loop
# closes in a 30-minute demo without needing force-retrain.
# Set DEMO_MODE=1 before starting Python: export DEMO_MODE=1
DEMO_MODE = os.environ.get("DEMO_MODE", "0") == "1"

# ---------------------------------------------------------------------------
# PATHS  (P0-1 FIX: use __file__-relative absolute paths pointing to
# ML_algorithm/ — the same directory Vite reads/writes. Running Python
# from any working directory will now resolve to the correct shared files.)
# ---------------------------------------------------------------------------
# Canonical shared directory: <repo>/ML_algorithm/
_REPO_ROOT   = Path(__file__).resolve().parent          # Ilevia-ai/
_ML_DIR      = _REPO_ROOT / "ML_algorithm"              # Ilevia-ai/ML_algorithm/

# All file-bus artefacts live under ML_algorithm/ so Vite and Python share them.
_ML_DIR.mkdir(exist_ok=True)
BASE_DIR      = _ML_DIR / "smarthome_data"
...
class SmartHomeSchemaValidator:
    ...
    # Zone 1 State, then remove the irrigation patch.
    #
    # Fix 1.2: Color Temperature is trained as a 3-class classifier
    # (Warm/Neutral/Cool) but the schema CSV parses it as numeric
    # (Integer 2000K–6500K). Patch both LED devices so the validator
    # resolves the string label instead of doing a numeric range check.
    _SCHEMA_PATCH: dict[tuple[str, str], list[str]] = {
        ("Smart Irrigation Controller", "Zone 1 State"): ["Watering", "Idle"],
        ("Smart LED Light",             "Color Temperature"): ["Warm", "Neutral", "Cool"],
    }
...
class SmartHomePredictionEngine:
    ...
    def predict_for_context(self, context_df: pd.DataFrame,
                             all_model_keys: list[str] | None = None,
                             model_filter: set | None = None) -> list[dict]:
        """
        ...
        Fix 1.5: model_filter — optional set of (device_id, param) tuples.
               When provided, only models whose (device_id, param) is in the
               set are dispatched. This replaces the DaySimulator pattern of
               temporarily swapping self.bank.models (which caused a race
               condition with the production _prediction_loop thread).
        """
        ...
        # Fix 1.5: model_filter applied here — no mutation of self.bank.models.
        with ThreadPoolExecutor(max_workers=min(32, len(self.bank.models))) as pool:
            futures = {
                ...
                or (bundle["device_id"], bundle["param"]) in model_filter
            }
...
class SmartHomeIncrementalLearner:
    ...
    def retrain(self, new_row_count: int = 0) -> SmartHomeModelBank:
        ...
        full_df["Timestamp"]     = pd.to_datetime(full_df["Timestamp"], format="mixed", utc=True).dt.tz_localize(None)
        ...
        # Fix 1.1: Build a content-hash → weight map from the FULL dataframe
        # BEFORE calling fit_transform(). fit_transform() calls _filter_human()
        # which drops automation rows and calls reset_index(drop=True), so the
        # original indices from full_df become invalid. Using a hash map lets us
        # re-join weights onto the filtered+reindexed processed frame by content.
        weight_map_hash: dict[str, float] = {}
        if "sample_weight" in full_df.columns:
            def _row_hash(row):
                s = "|".join(str(row.get(c, "")) for c in
                             ["Timestamp", "Device_ID", "Parameter_Changed", "New_Value"])
                return hashlib.md5(s.encode()).hexdigest()
            for _, row in full_df.iterrows():
                weight_map_hash[_row_hash(row)] = float(row["sample_weight"])

        processed = self.preprocessor.fit_transform(full_df, self.enricher)
        self.preprocessor.save()

        # Fix 1.1: Re-apply weights using content-hash lookup on the processed frame.
        # The old approach used weight_map.reindex(processed.index) which always
        # returned NaN after the index reset, making fillna(1) set all weights to 1.
        if weight_map_hash:
            def _get_weight(row, wm=weight_map_hash):
                s = "|".join(str(row.get(c, "")) for c in
                             ["Timestamp", "Device_ID", "Parameter_Changed", "New_Value"])
                return wm.get(hashlib.md5(s.encode()).hexdigest(), 1.0)
            processed = processed.copy()
            processed["sample_weight"] = processed.apply(_get_weight, axis=1)
...
def build_context_from_log(log_df: pd.DataFrame,
    ...
    Fix 2.6: as_of parameter lets callers (e.g. /api/simulate) stamp context
    rows with a simulated time instead of datetime.now(), ensuring that
    temporal features (Hour_of_Day, Is_Weekend, Month) are derived from the
    correct simulated timestamp rather than the real current time.
    """
    log_df = log_df.copy()
    log_df["Timestamp"] = pd.to_datetime(log_df["Timestamp"], format="mixed", utc=True).dt.tz_localize(None)
    ...
    latest["Timestamp"]      = as_of if as_of is not None else datetime.now()
...
class DaySimulator:
    ...
    def tick(self, sim_ts: datetime) -> dict:
        ...
        # Fix 1.5: Pass model_filter to predict_for_context instead of mutating
        # the shared engine bank. The old swap pattern caused a race condition
        # with the production _prediction_loop thread.
        raw_commands = self.engine.predict_for_context(ctx_df, model_filter=due_keys)
...
def run_full_demo(schema_csv: str,
    ...
    full_df["Timestamp"] = pd.to_datetime(full_df["Timestamp"], utc=True).dt.tz_localize(None)
    full_df = full_df.sort_values("Timestamp").reset_index(drop=True)
    total   = len(full_df)
...
class MLApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/api/deep-metrics':
            runtime = self.server.runtime
            learner = runtime._learner
            
            last_retrain = getattr(learner, 'last_retrain_time', None)
            log_df = pd.read_csv(LOG_STORE) if LOG_STORE.exists() else pd.DataFrame()
            total_samples = len(log_df)
            
            # Global accuracy average from metrics
            metrics_df = pd.read_csv(METRICS_STORE) if METRICS_STORE.exists() else pd.DataFrame()
            if not metrics_df.empty and 'metric' in metrics_df.columns:
                avg_acc = metrics_df[metrics_df['metric_name'] == 'Accuracy']['metric'].mean()
                if pd.isna(avg_acc): avg_acc = None
            else:
                avg_acc = None

            self._send_json(200, {
                "last_retraining_time": last_retrain.isoformat() if last_retrain else None,
                "total_training_samples": total_samples,
                "feature_count": len(FEATURE_COLS),
                "global_accuracy_average": float(avg_acc) if avg_acc is not None else None
            })
        else:
            self.send_error(404)
    ...
    def do_POST(self):
        parsed = urlparse(self.path)
        if parsed.path == '/api/force-retrain':
            runtime = self.server.runtime
            print("[API] Force retrain triggered.")
            runtime._run_learning_cycle()
            ...
            if engine is None or not LOG_STORE.exists():
                self._send_json(400, {"error": "Engine not ready"})
                return

            log_df = pd.read_csv(LOG_STORE)

            # Construct simulated timestamp BEFORE building context so as_of is defined
            now = datetime.now()
            hour = int(params.get('hour', now.hour))
            minute = int((float(params.get('hour', now.hour)) % 1) * 60)
            try:
                sim_time = now.replace(hour=hour, minute=minute, second=0, microsecond=0)
            except ValueError:
                sim_time = now

            # Fix 2.6: stamp context with simulated time from the start so
            # temporal feature derivation (Hour_of_Day etc.) uses sim_time.
            ctx_df = build_context_from_log(log_df, as_of=sim_time)
            
            if ctx_df.empty:
                self._send_json(400, {"error": "No context available"})
                return

            # We bypass enricher API call by forcing weather columns manually
            weather = params.get('weather', {})
            ctx_df['Outdoor_Temp_C'] = weather.get('temp', 25.0)
            ctx_df['Humidity_Pct'] = weather.get('humidity', 60.0)
            ctx_df['Wind_Speed_kmh'] = weather.get('wind', 10.0)
            ctx_df['Cloud_Cover_Pct'] = 50.0
            ctx_df['UV_Index'] = 3.0
            ctx_df['Is_Raining'] = 1 if 'rain' in weather.get('condition', '').lower() else 0
            ctx_df['Feels_Like_Temp_C'] = weather.get('feels', 25.0)
            
            # Solar features roughly based on daylight flag
            daylight = 1 if weather.get('daylight', True) else 0
            ctx_df['Is_Daylight'] = daylight
            ctx_df['Minutes_Since_Sunset'] = 0.0 if daylight else 120.0
            ctx_df['Minutes_To_Sunrise'] = 360.0 if daylight else 120.0

            # Fix 2.5: Inject Occupancy_Proxy from roomOccupancy parameter.
            # Previously the simulation endpoint silently ignored occupancy/roomOccupancy
            # because they weren't in FEATURE_COLS. Now we derive a simple binary
            # Occupancy_Proxy per device row so models can distinguish occupied/empty rooms.
            room_occupancy = params.get('roomOccupancy', {})
            occupancy_flag = params.get('occupancy', {})
            global_occupied = occupancy_flag.get('Occupancy', False) or occupancy_flag.get('Presence', False)

            def _device_room(device_id: str) -> str:
                if device_id.startswith('Bedroom'):   return 'Master Bedroom'
                if device_id.startswith('Kitchen'):   return 'Kitchen'
                if device_id.startswith('Garden'):    return 'Garden'
                if device_id.startswith('Entrance') or device_id.startswith('FrontDoor'):
                    return 'Entrance'
                return 'Living Room'

            occupancy_proxy = []
            for _, row in ctx_df.iterrows():
                room = _device_room(str(row.get('Device_ID', '')))
                occupied = global_occupied or room_occupancy.get(room, False)
                occupancy_proxy.append(1 if occupied else 0)
            ctx_df['Occupancy_Proxy'] = occupancy_proxy
            
            commands = engine.predict_for_context(ctx_df, all_model_keys=all_keys)
            
            # Filter commands to only show what changed significantly or what was confident
            deltas = []
            for c in commands:
                if c.get('valid') and not c.get('suppressed') and not c.get('no_model'):
                    deltas.append({
                        "device_id": c.get("device_id"),
                        "parameter": c.get("parameter"),
                        "simulated_value": c.get("validated_value", c.get("safe_value")),
                        "reason": c.get("reason", c.get("schema_msg", "Simulation output")),
                        "confidence": c.get("confidence", 0.7)
                    })

            self._send_json(200, {"deltas": deltas})
        else:
            self.send_error(404)

class MLApiServer(threading.Thread):
    def __init__(self, runtime, port=None):
        super().__init__(name="SmartHome-MLApiServer", daemon=True)
        # Fix 1.4: use ML_PORT constant (read from env at module level)
        self.port = port if port is not None else ML_PORT
        self.runtime = runtime
        self.server = HTTPServer(('0.0.0.0', self.port), MLApiHandler)
...
class SmartHomeRuntime:
    ...
    def __init__(
        ...
        schema_csv:           str,
        activity_log_csv:     str,
        # P0-1 FIX: default paths now point to ML_algorithm/ via _ML_DIR so
        # Python and Vite share the same files regardless of working directory.
        commands_out_path:    str  = str(_ML_DIR / "commands.json"),
        override_events_path: str  = str(_ML_DIR / "override_events.json"),
        latitude:             float = 19.0760,
        longitude:            float = 72.8777,
        api_key:              str | None = None,
        predict_interval_sec: int  = 15 * 60,
        retrain_interval_sec: int  = 60 * 60,
        command_log_path:     str  = str(_ML_DIR / "device_command_log.json"),
    ):
        self.schema_csv            = schema_csv
    ...
    def start(self):
        ...
        self._api_server = MLApiServer(self)
        self._api_server.start()
        
        print("[Runtime] Both loops and API server running. Press Ctrl+C to stop.\n")
    ...
    def _run_prediction_cycle(self):
        ...
        # P2-3 FIX: Inject Occupancy_Proxy in the live prediction loop.
        # The /api/simulate endpoint injects this from the UI; without it
        # the live 15-min cycle lacked occupancy context and produced less
        # accurate predictions for models trained with this feature.
        # Heuristic: mark a device's room as "occupied" if any human event
        # occurred for a device in that room within the last 30 minutes.
        def _room_from_device(device_id: str) -> str:
            if device_id.startswith("Bedroom"):  return "Master Bedroom"
            if device_id.startswith("Kitchen"):  return "Kitchen"
            if device_id.startswith("Garden"):   return "Garden"
            if device_id.startswith("Entrance") or device_id.startswith("FrontDoor"):
        ...
        if "Timestamp" in log_df.columns and "Trigger_Source" in log_df.columns:
            now_ts     = datetime.now()
            cutoff     = now_ts - timedelta(minutes=30)
            recent_occ = set()
            try:
                ts_col = pd.to_datetime(log_df["Timestamp"], format="mixed", utc=True).dt.tz_localize(None)
                recent = log_df[
                    (ts_col >= pd.Timestamp(cutoff)) &
                    (log_df["Trigger_Source"] == "Human")
                ]
                for _, r in recent.iterrows():
                    recent_occ.add(_room_from_device(str(r.get("Device_ID", ""))))
            except Exception:
                pass
            occ_proxy = []
            for _, row in ctx_df.iterrows():
                room = _room_from_device(str(row.get("Device_ID", "")))
                occ_proxy.append(1 if room in recent_occ else 0)
            ctx_df["Occupancy_Proxy"] = occ_proxy
    ...
    def _run_learning_cycle(self):
        ...
        # P2-4 FIX: Clear override file AFTER successful append, not before read.
        self._clear_override_events()
        ...
        # DEMO_MODE: threshold lowered from 10 → 1 so the loop closes in a demo
        min_rows = 1 if DEMO_MODE else 10
        should_retrain = n_added >= min_rows or n_corrections > 0
    ...
    def _consume_override_events(self) -> list[dict]:
        """
        Read override_events_path written by the frontend.

        P2-4 FIX: File is cleared AFTER events are successfully appended to
        the activity log, not before. The old pattern cleared the file
        immediately after reading — a crash between read and append_new_data()
        would permanently destroy all pending events.

        This method only reads; the caller is responsible for clearing the file
        after a successful append by calling _clear_override_events().
        """
        ...
        try:
            ...
            if not isinstance(events, list) or not events:
                return []
            # DO NOT clear here — cleared by _clear_override_events() after
            # successful append_new_data() so events are never silently lost.
            return events
        except (json.JSONDecodeError, IOError) as e:
            print(f"[LearnLoop] Could not read override events: {e}")
            return []

    def _clear_override_events(self) -> None:
        """Mark override file as consumed. Called only after successful append."""
        try:
            self.override_events_path.write_text("[]", encoding="utf-8")
        except IOError as e:
            print(f"[LearnLoop] WARNING: could not clear override_events.json: {e}")
    ...
    def _count_corrections(new_df: pd.DataFrame) -> int:
        ...
        df = new_df.copy()
        df["Timestamp"] = pd.to_datetime(df["Timestamp"], format="mixed", errors="coerce", utc=True).dt.tz_localize(None)
...
if __name__ == "__main__":
    ...
    # P0-1: CSV files live in the repo root (same dir as smart_home_pipeline.py)
    SCHEMA_CSV       = str(_REPO_ROOT / "Smart_Home_IoT_Parameters-v2.csv")
    ACTIVITY_LOG_CSV = str(_REPO_ROOT / "activity_log_weather.csv")
    ...
    if MODE == "production":
        ...
        # On subsequent runs: loads existing models from ML_algorithm/smarthome_data/models/.
        #
        # The frontend should:
        #   WRITE → ML_algorithm/override_events.json  (human device interactions)
        #   READ  ← ML_algorithm/commands.json         (validated model predictions)
        print("\n[Main] Starting in PRODUCTION mode...")
        if DEMO_MODE:
            print("[Main] DEMO_MODE=1 — retrain threshold lowered to 1 row.")
        runtime = SmartHomeRuntime(
            schema_csv           = SCHEMA_CSV,
            activity_log_csv     = ACTIVITY_LOG_CSV,
            # Paths resolved via _ML_DIR defaults — no need to override here
